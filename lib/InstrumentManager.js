        ...
        this.instrumentConfig.forEach((instrumentData) => {
            if (instrumentData.enabled) {
                if (instrumentData.instrumentClass == 'AudioInstrument') {
                    let instrument = new AudioInstrument_1.AudioInstrument(this.audioContext, this.masterVolumeGainNode, rootPath, instrumentData);
                    this.instruments.set(instrument.id, instrument);
                }
                else if (instrumentData.instrumentClass == 'SoundfontInstrument') {
                    let instrument = new SoundfontInstrument_1.SoundfontInstrument(this.audioContext, this.masterVolumeGainNode, rootPath, instrumentData);
                    this.instruments.set(instrument.id, instrument);
                }
            }
        });
